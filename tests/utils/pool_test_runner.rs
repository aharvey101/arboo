...
// Individual pool test functions
...
pub async fn run_pool_cache_tests() -> Result<()> {
    ...
    // Run multiple cache-related tests
    let tests = vec![
...
pub async fn run_pool_discovery_tests() -> Result<()> {
    ...
    // Test pool discovery infrastructure without hitting rate limits
...
async fn test_pool_discovery_infrastructure() -> Result<()> {
    ...
    let config = TestEnvironmentConfig {
        ...
        websocket_port: None,
        enable_logging: false, // Reduce noise for this test
        gas_limit: 21000,
    ...
    // Test 5: Pool filtering and search
...
fn create_test_pool_set() -> Vec<arbooo::common::pools::Pool> {
    ...
    vec![
        ...
        // WBTC/WETH pair (V3 only - no arbitrage opportunity)
        Pool {
            id: 5,
        ...
        // DAI/USDC pair (V2 only - no arbitrage opportunity)
        Pool {
            id: 6,
...
fn identify_arbitrage_pairs(pools: &[arbooo::common::pools::Pool]) -> Vec<(arbooo::common::pools::Pool, arbooo::common::pools::Pool)> {
    ...
    // Find pairs where we have both V2 and V3 pools
    let mut arbitrage_pairs = Vec::new();
    
    for (_, pool_group) in token_pairs {
        if pool_group.len() >= 2 {
            // Look for V2/V3 combinations
